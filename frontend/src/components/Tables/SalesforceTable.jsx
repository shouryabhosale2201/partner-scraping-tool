...
const FilterSidebar = ({ selectedFilters, setSelectedFilters, onFilterChange }) => {
    ...
    useEffect(() => {
        if (searchTerm) {
            setOpenSections(prev => {
                ...
                filtersBySection.forEach(({ section, filters }) => {
                    const sectionKey = normalizeSectionKey(section);
                    if (section === "Country") {
                        const countryMatch = filters.some(country => {
                            const regionMatch = country.children.some(region =>
                                region.toLowerCase().includes(searchTerm.toLowerCase())
                            );
                            if (regionMatch) {
                                newState[section] = true;
                                foundMatch = true;
                                return true; // Stop iterating countries
                            }
                            return regionMatch;
                        });
                        if (!countryMatch) {
                        ...
                        const itemMatch = filters.some(item =>
                            item.toLowerCase().includes(searchTerm.toLowerCase())
                        );
                        newState[section] = itemMatch;
                        if (itemMatch) {
                            foundMatch = true;
                        } else {
                            newState[section] = true;
    ...
    const renderSection = (section, filters) => {
        ...
        const hasMatchingItems = !isNested ? filteredItems.length > 0 : filteredItems.some(c => c.children.length > 0);
                    ...
                    <div className="flex flex-col gap-2 pl-2 mt-2">
                        {isNested ? (
                            filteredItems.map((country, i) => {
                                const countryHasMatchingRegions = country.children.some(region => region.toLowerCase().includes(searchTerm.toLowerCase()));
                                return (
                                    <div key={i} className="pl-2">
                                        <h4 className="font-medium mb-1">{country.label}</h4>
                                        <div className="flex flex-col gap-1 pl-4">
                                            {countryHasMatchingRegions ? (
